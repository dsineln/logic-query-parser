...
module.exports.stringSyntaxer = function stringSyntaxer(options, lexemesArray, i, tree) {
  ...
  block.left = null;
  block.right = null;
  //console.log(`DEBUG___ string syntaxer after ${JSON.stringify(tree)}`);
  return i + 1;
};
...
function generateBasicSyntaxer(type) {
  return function basicSyntaxer(options, lexemesArray, i, tree) {
    ...
    //console.log(`DEBUG___ basic syntaxer, type = ${type}`);
    if(!block.lexeme) {
      throw new Error("Nothing before " + type + " block");
    }

    //console.log(`DEBUG___ genericSyntaxer, ${JSON.stringify(lexemesArray[i])}, BLOCK: ${JSON.stringify(block)}`);
...
module.exports.commaSyntaxer = generateBasicSyntaxer('comma');
module.exports.inSyntaxer = generateBasicSyntaxer('in');
...
module.exports.notSyntaxer = function notSyntaxer(options, lexemesArray, i, tree) {
  ...
  block.left = {};
  block.right = null;
  //console.log(`DEBUG___ not syntaxer after ${JSON.stringify(tree)}`);
  return i + 1;
};
...
function genericBlockSyntaxer(typeStart, typeEnd) {
  return function basicSyntaxer(options, lexemesArray, i, tree, createTree) {
    ...
    block.left = newBlock;
    block.right = null;
    //console.log(`DEBUG___ block syntaxer after ${JSON.stringify(tree)}`);
  }
};
